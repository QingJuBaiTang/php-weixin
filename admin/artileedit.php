...
<form action="<?= $action ?>" method="post" name="newsform">
    ...
    <div class="formbody">
        ...
        <ul class="forminfo">
            <li><label>选择类型</label>
                <div class="vocation">
                    <select class="select1" name="type"
                            onchange="if(this.value==2){document.getElementById('jg').style.display=''}else{document.getElementById('jg').style.display='none'}">
                        <?
            ...
            <li><label>名称</label><input name="name" type="text" class="dfinput" value="<?= $row['name'] ?>"/><i></i>
            </li>
            <li><label>缩略图</label><input name="img" type="text" id="upImgIpt" class="dfinput" value="<?= $row['img'] ?>"
                                         value="<?= $row['img'] ?>"/><i><input id="upImgBtn" type="button" value="选择图片"
                                                                               class="scbtn"></i></li>
            <li id="jg" style="<? if ($row['type'] == 2) {
                echo '';
            } else {
                echo 'display:none';
            } ?>"><label>价格</label><input name="price" type="text" class="dfinput"
                                          value="<?= $row['price'] ?>"/><i>元</i></li>
            <li><label>描述</label><input name="sdesc" type="text" class="dfinput" value="<?= $row['sdesc'] ?>"/></li>
            <li><label>详情</label><textarea name="Content"
                                           style="width:670px;height:350px;visibility:hidden;"><?php echo $row['content'] ?></textarea>
            </li>
