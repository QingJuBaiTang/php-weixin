...
<div class="rightinfo">
            ...
            <li style="float:right">
                <form name="searchsoft" method="post" action="?act=search&t=<?= $_GET['t'] ?>&s=<?= $_GET['s'] ?>">
                    ...
                    <input name="title" type="text" id="title" size="20" maxlength="50" class="dfinput"
                           style="width:200px;height:28px" placeholder="标题或内容"/>
    ...
    <form method="post" action="?" id="forms" name="forms">
        <table class="imgtable">
            <thead>
            <tr>
                <th width="50px;"><input name="chkAll" type="checkbox" id="chkAll" onclick=CheckAll(this.form)
                                         value="checkbox"/></th>
                <th>标题</th>
                <th>用户ID</th>
                <th>广告图片</th>
            ...
            while ($row = mysql_fetch_array($query)) {
                ?>
                <tr height="35px">
                    <td><input name='ids[]' type='checkbox' onClick="unselectall()" id="ID" value='<?= $row['id'] ?>'>
                    </td>
                    <td><a href="../view.php?fid=<?= $row['infoid'] ?>"
                           target="_blank"><?php echo urldecode($row['title']); ?></a></td>
                    <td><?= $row['userid'] ?></td>
                    <td><a href="<?= $row['adlink'] ?>" target="_blank"><img src="../<?= $row['adpic'] ?>" width="260"
                                                                             style="max-height:60px;"></a></td>
                    <td><?= $row['wcount'] ?></td>
                    <td><?= $row['acount'] ?></td>
                    <td>
                        <a href="artile.php?id=<?= $row['id'] ?>&act=del&t=<?= $_GET['t'] ?>&page=<?= $_GET['page'] ?>&s=<?= $_GET['s'] ?>&userid=<?= $_GET['userid'] ?>"
                           onclick="if (confirm('确定要删除吗？')) return true; else return false;" class="tablelink"> 删除</a>
                    </td>
                </tr>
                <?
            }
