        ...
        <table class="imgtable">
            ...
            while ($row = mysql_fetch_array($query)) {
                ...
                <tr height="35px">
                    ...
                    <td>
                        <a href="adverlist.php?id=<?= $row['id'] ?>&act=del&t=<?= $_GET['t'] ?>&page=<?= $_GET['page'] ?>&s=<?= $_GET['s'] ?>&userid=<?= $_GET['userid'] ?>"
                           onclick="if (confirm('确定要删除吗？')) return true; else return false;" class="tablelink"> 删除</a>
                        <?
                        if ($row['systemdefault'] == 0) {
                            ?>
                            <font color="#FF0000"><a
                                        href="adverlist.php?id=<?= $row['id'] ?>&act=setdefault&t=<?= $_GET['t'] ?>&page=<?= $_GET['page'] ?>&s=<?= $_GET['s'] ?>&userid=<?= $_GET['userid'] ?>"
                                        onclick="if (confirm('确定要设置默认广告吗？')) return true; else return false;"
                                        class="tablelink"> 设为默认广告</a>
                            </font>
                            <?
                        } else {
                            ?>
