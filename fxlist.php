...
.list_txt h6 {padding-left:10px;max-width:90%;color:#333; font-size:14px; padding-top:5px; line-height:16px; height:24px; overflow: hidden; }
...
<div class="apply" id="apply">
	...
	<form action="?act=search" id="signupok" method="get">
		...
		<dl class="clearfix" style="width:50%; float:left; padding-top:10px">
			<dd>
				<input type="text" class="input_txt sel" value='' name="title" id="adtitle" placeholder="请输入标题关键字" style="width:100%">
			</dd>
		</dl>
		<div class="btn_box">
...
<ul class="am-avg-sm-3 am-text-center admin-content-list">
    <li class="am-text-success">
      <a href="./index.php?i=257&c=entry&do=list&m=weisrc_quickad" class="am-text-success"><span class="am-icon-btn am-icon-file-text"></span><br/>
    文章总数<br/>
   <?=$rowa['cc']?>    </a>
    </li>
  ...
  <li class="am-text-danger"><span class="am-icon-btn am-icon-recycle"></span><br/>
	...
	<div class="list1" style="margin-bottom:50px;">
		<br>
		<dd><div style='margin-left:8px; margin-top:-5px; margin-right:10px; font-size:14px; color:#FF0000;'>温馨提示：已经分享出去的内容千万不能删除，否则用户在阅读分享的文章将会消失。</div></dd>
		<ul>
			<?
			if($_GET['act']='search') {
			...
			<li>
				<div class="list_txt list_t">
					<a  class="titlelist" href="view.php?fid=<?=$row['infoid']?>"><?=$row['id']?>、<?php echo urldecode($row['title']);?></a><br>阅读:<?=$row['wcount']?>&nbsp;&nbsp;广告点击:<?=$row['acount']?>&nbsp;&nbsp;点赞:<?=$row['zan']?> 
					<a  style="float:right; padding-left:15px" href="fxlist.php?infoid=<?=$row['infoid']?>&act=del&page=<?=$_GET['page']?>" onClick="if (confirm('确定要删除吗？')) return true; else return false;">删除</a>
					<a style="float:right" href="fxedit.php?fid=<?=$row['infoid']?>" onClick="if (confirm('确定要编辑吗？')) return true; else return false;">编辑</a>
				</div>
			</li>
			<?
				$count++;
